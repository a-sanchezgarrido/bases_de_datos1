...
CREATE TABLE chat_grupo(
    ...
    msj_anclado CHAR(10), -- NOT NULL lo quitamos porque si no, no podemos insertar nada
...
CREATE TABLE mensaje(
  ...
  CONSTRAINT mensaje_fk_perfil FOREIGN KEY (perfil) REFERENCES perfil(movil),
  -- ON DELETE NO ACTION ON UPDATE CASCADE
  CONSTRAINT mensaje_fk_chat_grupo FOREIGN KEY (chat_grupo) REFERENCES chat_grupo(codigo),
  -- ON DELETE CASCADE
...
CREATE TABLE participacion(
  ...
  CONSTRAINT participacion_pk PRIMARY KEY (perfil, chat_grupo),
  CONSTRAINT participacion_fk_perfil FOREIGN KEY (perfil) REFERENCES perfil(movil),
  -- ON DELETE CASCADE
  CONSTRAINT participacion_fk_chat_grupo FOREIGN KEY (chat_grupo) REFERENCES chat_grupo(codigo)
  -- ON DELETE CASCADE
);
...
CREATE TABLE email_contacto(
  ...
  CONSTRAINT emailc_pk PRIMARY KEY (contacto, perfil, email),
  CONSTRAINT emailc_fk_contacto FOREIGN KEY (contacto, perfil) REFERENCES contacto(telefono, perfil)
  -- ON DELETE CASCADE
);
